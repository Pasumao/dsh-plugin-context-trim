...
export class GateCatalog {
  constructor(ctx, store) {
    ...
    /** 调试：最近一次真实组装的快照（scope 判定 + 模型实际收到的工具名）。 */
      ...
      try {
        if (assembly !== null && typeof assembly === 'object') {
          ...
          this.lastAssembly = {
            ...
            toolNames: (Array.isArray(assembly.tools) ? assembly.tools : []).map((t) => t?.name).filter(Boolean),
            sections: (Array.isArray(assembly.sections) ? assembly.sections : []).map((s) => s?.name).filter(Boolean)
          }
        }
      } catch { /* 观测失败不影响组装 */ }
      return next()
